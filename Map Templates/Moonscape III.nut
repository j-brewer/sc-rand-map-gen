...
function Generate(){
	...
	local cutoffA = 55;
	local cutoffB = 87;
	local craterDepthOffset = 4096
	local peak = (cutoffB - centerpeak) * (cutoffB - centerpeak) - craterDepthOffset;
	local step = peak / (craterSize - cutoffB - 1)
	...
	for(local i =0; i < craterSize; i=i+1)
	{
		...
		{
			cj = i - centerpeak;
			crater.SetValue(i,  2*cj*cj - craterDepthOffset)
		}
		else if(i <= cutoffA)
		{
			ck = i - centerpeak;
			crater.SetValue(i, 36*ck - craterDepthOffset)
		}
		else if(i <= cutoffB)
		{
			cl = i - cutoffA + 6;
			crater.SetValue(i,  (3*cl*cl + crater.GetValue(cutoffA-3)).tointeger())
		}
		else
		{
			cm = i - cutoffB;
			crater.SetValue(i, crater.GetValue(cutoffB) - cm * (crater.GetValue(cutoffB)/ (craterSize - cutoffB - 1)))
		}
	}
	...
	for(local j = 0; j < craterCount; j++)
	{
		...
		if(IsValidCraterPosition(b1 + scaleFactor/2, b2+ scaleFactor/2, scaleFactor/2 + 5, startPos))
		{
